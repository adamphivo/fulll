...
Given("I am an application user named {string}", async function (userName: string) {
  const command = new CreateUserCommand(userName);
  const handler = new CreateUserHandler(this.userRepository, this.fleetRepository);
...
Given("I have registered this vehicle into my fleet", async function () {
  const command = new RegisterVehicleCommand(this.fleet.getId(), this.vehicle.getPlateNumber());
  const handler = new RegisterVehicleHandler(this.fleetRepository, this.vehicleRepository);
  await handler.handle(command);
});

Given("the fleet of another user named {string}", async function (userName: string) {
  // Create other user
  const command = new CreateUserCommand(userName);
  const handler = new CreateUserHandler(this.userRepository, this.fleetRepository);
  const user = await handler.handle(command);
...
Given("this vehicle has been registered into the other user's fleet", async function () {
  const command = new RegisterVehicleCommand(this.otherFleet.getId(), this.vehicle.getPlateNumber());
  const handler = new RegisterVehicleHandler(this.fleetRepository, this.vehicleRepository);
  await handler.handle(command);
});
...
Then("this vehicle should be part of my vehicle fleet", function () {
  assert.deepStrictEqual(this.result.getVehicles().includes(this.vehicle.getPlateNumber()), true);
});

Then("I should be informed that this vehicle has already been registered into my fleet", function () {
  assert.deepStrictEqual(this.result, Error(Configuration.ERROR_MESSAGES.DUPLICATE_VEHICLE));
});
